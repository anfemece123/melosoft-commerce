...
| Idioma | `es_CO` (español, Colombia). Si Meta no permite `es_CO` para la cuenta, usar `es` genérico y ajustar `META_GRAPH_API_VERSION`/`template_language` en `store_whatsapp_settings.locale` acorde. |
| Header | Ninguno |
| Footer | Ninguno |
