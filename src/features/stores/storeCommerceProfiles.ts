...
// Shared between StoreFormPage (creation) and StoreSettingsPage (editing
// an existing store) so both offer the exact same options per vertical —
// notably 'lociones_perfumes', the value FragrancePyramid gates on
// (see src/lib/storefront/fragrancePyramid.ts).

export const SUBCATEGORIES_BY_VERTICAL: Record<BusinessVertical, Array<{ value: string; label: string }>> = {
  food_restaurant: [
