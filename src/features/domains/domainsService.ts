...
// Custom-domain management is intentionally paused. Keep the implementation
// available for a future reactivation, but never call the Edge Function from
// the current panel while the feature is hidden.
const CUSTOM_DOMAIN_MANAGEMENT_ENABLED = false;
...
export const domainsService = {
  async list(storeId: string): Promise<StoreDomain[]> {
    if (!CUSTOM_DOMAIN_MANAGEMENT_ENABLED) return [];
  ...
  async connect(storeId: string, hostname: string): Promise<StoreDomain> {
    if (!CUSTOM_DOMAIN_MANAGEMENT_ENABLED) {
      throw new Error('La gestión de dominios personalizados no está disponible temporalmente.');
    }
  ...
  // While custom domains are paused, every link generated by the panel uses
  // the stable platform URL (included subdomain or the legacy /s/ route).
  getStorePublicUrl(storeSlug: string): string {
