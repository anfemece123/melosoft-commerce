...
describe('computeAllowedOrigins', () => {
  it('always includes local dev and both production admin origins', () => {
    ...
    expect(origins.has('https://admin.melosoftapp.com')).toBe(true);
    expect(origins.size).toBe(4);
  });

  it('treats null/empty PLATFORM_HOSTNAMES the same as undefined', () => {
    expect(computeAllowedOrigins(null).size).toBe(4);
    expect(computeAllowedOrigins('').size).toBe(4);
  });
