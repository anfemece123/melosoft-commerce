-- ============================================================
-- Migration 121 — Owner-controlled Carta cover and visual details
-- ============================================================

ALTER TABLE public.store_carta_settings
  ADD COLUMN cover_layout text NOT NULL DEFAULT 'none',
  ADD COLUMN cover_product_ids uuid[] NOT NULL DEFAULT '{}'::uuid[],
  ADD COLUMN show_logo boolean NOT NULL DEFAULT true,
  ADD COLUMN show_product_descriptions boolean NOT NULL DEFAULT true,
  ADD COLUMN category_heading_alignment text NOT NULL DEFAULT 'center',
  ADD CONSTRAINT store_carta_cover_layout_valid
    CHECK (cover_layout IN ('none', 'single', 'collage')),
  ADD CONSTRAINT store_carta_cover_product_limit
    CHECK (cardinality(cover_product_ids) <= 3),
  ADD CONSTRAINT store_carta_category_heading_alignment_valid
    CHECK (category_heading_alignment IN ('left', 'center'));
