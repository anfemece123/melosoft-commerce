-- ============================================================
-- Migration 047 — Images per variant option value (e.g. Color)
--
-- Problem: with images only at product-level or exact-variant-level,
-- a shoe sold in 3 colors × 4 sizes needed the SAME color photo
-- uploaded 4 times (once per size) — tedious and not how real
-- ecommerce (AliExpress, Shopify, etc.) works.
--
-- Fix: product_images gains a second, mutually-exclusive optional
-- owner — option_value_id — so a photo can be attached to "Color:
-- Verde" once and used by every size of that color. Priority at
-- render time (exact variant image > selected visual option's
-- images > general product gallery) is resolved entirely client-side
-- (see productVariants.utils.ts) — this migration only adds the
-- pieces the client needs to do that.
--
-- Additive only: no existing column, row, or view behavior changes
-- for products that don't use this (variant_id-only or general
-- images keep working exactly as before).
-- ============================================================

-- ── 1. product_images.option_value_id ─────────────────────────

ALTER TABLE public.product_images
  ADD COLUMN IF NOT EXISTS option_value_id uuid
    REFERENCES public.product_variant_option_values(id) ON DELETE CASCADE;

-- An image is general (both null), exact-variant (variant_id set), or
-- option-value (option_value_id set) — never both of the latter two.
ALTER TABLE public.product_images
  DROP CONSTRAINT IF EXISTS product_images_variant_or_option_value_exclusive;
ALTER TABLE public.product_images
  ADD CONSTRAINT product_images_variant_or_option_value_exclusive
  CHECK (NOT (variant_id IS NOT NULL AND option_value_id IS NOT NULL));

CREATE INDEX IF NOT EXISTS idx_product_images_option_value_id
  ON public.product_images(option_value_id) WHERE option_value_id IS NOT NULL;

-- ── 2. product_variant_options.controls_media ─────────────────
-- Marks which option (usually Color/Modelo) drives the product
-- gallery when selected. Admin-set explicitly (rather than inferred
-- from "has any option-value images") so the owner can flag the
-- option and start uploading images for its values right away.

ALTER TABLE public.product_variant_options
  ADD COLUMN IF NOT EXISTS controls_media boolean NOT NULL DEFAULT false;

-- ── 3. public_product_pages — expose controls_media + per-value
--      image galleries. Additive to migration 046's definition:
--      only the `voptions` lateral's per-option/per-value objects
--      gain new keys. variants/facet_values/collections untouched.
