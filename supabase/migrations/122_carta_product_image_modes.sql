-- ============================================================
-- Migration 122 — Product image visibility modes for Carta
-- ============================================================

ALTER TABLE public.store_carta_settings
  ADD COLUMN product_image_mode text NOT NULL DEFAULT 'all',
  ADD CONSTRAINT store_carta_product_image_mode_valid
    CHECK (product_image_mode IN ('all', 'first_per_category', 'none'));

COMMENT ON COLUMN public.store_carta_settings.product_image_mode IS
  'Controls product imagery inside carta categories: every product, one category opener, or no images. Cover imagery is configured independently.';
