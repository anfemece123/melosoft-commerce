-- A parent category with subcategories previously had no orderable product set of its
-- own: store_catalog_product_positions only accepted a position row whose category_id
-- matched the product's own (leaf) category exactly, so "Orden del catálogo" showed
-- nothing for a parent category and the storefront's "Todas" (no subcategory) view fell
-- back to creation-date order. This lets a position be keyed to a product's direct
-- parent category too, giving merchants one flat, mergeable order across all of a parent
-- category's subcategories, independent from each subcategory's own order.
  ...
  if new.category_id is not null and not exists (
    ...
    where product.id = new.product_id
      ...
      and (
        ...
        or product_category.parent_id = category.id
      )
  ) then
...
-- Returns the complete ordered list for one admin merchandising context. When
-- p_category_id is a parent category, the returned set now also includes products
-- assigned directly to any of its direct subcategories (a flat, merged order).
...
begin
  ...
  where product.store_id = p_store_id
    ...
    and (
      ...
      or product_category.parent_id = p_category_id
    )
    and (
...
-- Atomically validates and persists the complete order for one context. When
-- p_category_id is a parent category, the required product set now includes products
-- assigned to its direct subcategories too, matching get_store_catalog_product_order.
     ...
     or exists (
       ...
       where product.id is null
          ...
          or (
            ...
            and coalesce(product_category.parent_id <> p_category_id, true)
          )
...
-- Browsing a specific subcategory keeps using that subcategory's own order; browsing a
-- parent category with no subcategory chosen ("Todas") now uses the parent's own flat,
-- merged order instead of concatenating each subcategory's order one after another.
