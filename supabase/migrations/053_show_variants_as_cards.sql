-- ============================================================
-- Migration 053 — per-product "show visual variants as cards"
--
-- Lets the owner choose, per product, whether the public catalog
-- splits its controls_media option (usually Color/Modelo) into one
-- card per value (fashion/footwear style) or keeps the current
-- single grouped card with swatches. Per-product rather than
-- store-wide, since some products in the same store benefit from
-- splitting and others don't (per-product config, as requested).
--
-- Purely additive: default false keeps every existing product's
-- catalog card exactly as it renders today.
-- ============================================================

ALTER TABLE public.products
  ADD COLUMN IF NOT EXISTS show_variants_as_cards boolean NOT NULL DEFAULT false;

-- public_product_pages — additive column, identical to 047's
